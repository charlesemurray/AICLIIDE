...
async fn test_tool_manager_skill_discovery() {
    ...
    assert!(
        ...
        "ToolManager should have registered tools"
    );

    // Step 3: Verify skill registry is accessible
    assert!(
        !tool_manager.skill_registry.is_empty(),
        "Skill registry should have skills"
    );
}
...
async fn test_workflow_with_variable_interpolation() {
    ...
    assert!(
        ...
        "Workflow should execute (may fail on interpolation)"
    );
}
