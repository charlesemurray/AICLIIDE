use chat_cli::cli::chat::tools::{Tool, ToolManager};
...
async fn test_skill_discoverable_by_agent() {
    ...
    assert!(calculator_tool.is_some(), "Calculator skill should be discoverable as a tool");
}
...
async fn test_skill_invocation_through_tool_interface() {
    ...
    if let Some(Tool::Skill(skill_tool)) = calculator_tool {
        ...
        assert!(output.to_string().contains("8"), "Calculator should return correct result");
    } else {
...
async fn test_multiple_skills_registered() {
    ...
    let tools = tool_manager.get_all_tools();
    let skill_tools: Vec<_> = tools.iter()
        .filter(|t| matches!(t, Tool::Skill(_)))
        .collect();
