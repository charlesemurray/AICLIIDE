...
mod builder_integration_tests {
    ...
    async fn test_skill_creation_end_to_end() -> Result<()> {
        ...
        let mut flow = SkillCreationFlow::new("data-analyzer".to_string(), CreationMode::Guided)?.with_ui(Box::new(ui));
    ...
    async fn test_command_creation_end_to_end() -> Result<()> {
        ...
        let mut flow = CommandCreationFlow::new("quick-commit".to_string(), CreationMode::Quick)?.with_ui(Box::new(ui));
    ...
    async fn test_agent_creation_end_to_end() -> Result<()> {
        ...
        let mut flow = AgentCreationFlow::new("code-helper".to_string(), CreationMode::Expert)?.with_ui(Box::new(ui));
    ...
    async fn test_context_aware_creation() -> Result<()> {
        ...
        let mut flow = SkillCreationFlow::new("main-runner".to_string(), CreationMode::Quick)?.with_ui(Box::new(ui));
    ...
    async fn test_preview_mode_integration() -> Result<()> {
        ...
        let mut flow =
            CommandCreationFlow::new("preview-cmd".to_string(), CreationMode::Preview)?.with_ui(Box::new(ui));
    ...
    async fn test_validation_error_handling() -> Result<()> {
        ...
        // Try with valid name
        let mut ui = MockTerminalUI::new(vec!["echo valid".to_string(), "y".to_string()]);

        let mut flow = CommandCreationFlow::new("valid-cmd".to_string(), CreationMode::Quick)?.with_ui(Box::new(ui));
    ...
    async fn test_multiple_creation_modes() -> Result<()> {
        ...
        let mut quick_ui = MockTerminalUI::new(vec!["python quick.py".to_string()]);

        let mut quick_flow =
            SkillCreationFlow::new("quick-skill".to_string(), CreationMode::Quick)?.with_ui(Box::new(quick_ui));
        ...
        let mut guided_flow =
            SkillCreationFlow::new("guided-skill".to_string(), CreationMode::Guided)?.with_ui(Box::new(guided_ui));
    ...
    async fn test_builder_system_integration() -> Result<()> {
        ...
        // Create skill
        let mut skill_ui = MockTerminalUI::new(vec!["python process.py".to_string(), "y".to_string()]);
        let mut skill_flow =
            SkillCreationFlow::new("processor".to_string(), CreationMode::Quick)?.with_ui(Box::new(skill_ui));
        let skill_config = skill_flow.run_single_pass()?;

        // Create command
        let mut cmd_ui = MockTerminalUI::new(vec!["ls -la".to_string(), "y".to_string()]);
        let mut cmd_flow =
            CommandCreationFlow::new("list-all".to_string(), CreationMode::Quick)?.with_ui(Box::new(cmd_ui));
        let cmd_config = cmd_flow.run_single_pass()?;

        // Create agent
        let mut agent_ui = MockTerminalUI::new(vec!["You help with tasks".to_string(), "y".to_string()]);
        let mut agent_flow =
            AgentCreationFlow::new("helper".to_string(), CreationMode::Quick)?.with_ui(Box::new(agent_ui));
