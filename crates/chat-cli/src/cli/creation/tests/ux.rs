...
mod power_user_efficiency {
    ...
    fn test_preview_mode_no_creation() {
        let mut ui = MockTerminalUI::new(vec!["echo hello".to_string()]);
...
mod error_messaging {
    ...
    fn test_immediate_validation_feedback() {
        ...
        let result = ui
            .prompt_with_validation("Skill name", |name| {
                if name.contains('!') {
...
mod cognitive_load_management {
    ...
    fn test_consistent_interaction_patterns() {
        let mut ui = MockTerminalUI::new(vec!["test".to_string(), "y".to_string(), "n".to_string()]);
