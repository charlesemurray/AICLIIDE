use eyre::Result;
use crate::cli::creation::{TerminalUI, SemanticColor};

pub struct EnhancedPrompts;

impl EnhancedPrompts {
    pub fn prompt_skill_name<T: TerminalUI>(ui: &mut T) -> Result<String> {
        ui.show_message("💡 Skill names should be descriptive and use kebab-case (e.g., 'git-status', 'deploy-app')\n", SemanticColor::Info);
        ...
        loop {
            ...
            ui.show_message("❌ Invalid name. Use lowercase letters, numbers, and hyphens only.\n", SemanticColor::Error);
        }
    }

    pub fn prompt_command<T: TerminalUI>(ui: &mut T) -> Result<String> {
        ui.show_message("⚡ Enter the command that this skill will execute (e.g., 'git status', 'docker ps')\n", SemanticColor::Info);
        ...
        loop {
            ...
            ui.show_message("❌ Command cannot be empty or contain only whitespace.\n", SemanticColor::Error);
        }
    }

    pub fn prompt_description<T: TerminalUI>(ui: &mut T, item_type: &str) -> Result<Option<String>> {
        ui.show_message(&format!("📝 Provide a brief description of what this {} does\n", item_type), SemanticColor::Info);
        Ok(ui.prompt_optional("Description", None)?)
    }

    pub fn prompt_agent_role<T: TerminalUI>(ui: &mut T) -> Result<String> {
        ui.show_message("🤖 Define the agent's role (e.g., 'code reviewer', 'documentation writer', 'assistant')\n", SemanticColor::Info);
        ...
        loop {
            ...
            ui.show_message("❌ Role cannot be empty or contain only whitespace.\n", SemanticColor::Error);
        }
    }
