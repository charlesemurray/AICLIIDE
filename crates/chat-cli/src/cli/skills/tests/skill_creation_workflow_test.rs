...
"#;
        ...
        if has_test_skill {
            ...
            let execution_result = registry.execute_skill("test-workflow", json!({"message": "Workflow test successful!"})).await;
            ...
            if workspace_skills.is_empty() {
                println!("ℹ️ No workspace skills loaded - this may be expected if dynamic loading is not fully implemented");
            } else {
                println!("ℹ️ Found {} workspace skills: {:?}", 
