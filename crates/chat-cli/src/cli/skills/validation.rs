...
impl SkillValidator {
    pub fn validate_skill_json(content: &str) -> Result<Value, SkillError> {
        ...
        let json: Value =
            serde_json::from_str(content).map_err(|e| SkillError::InvalidInput(format!("Invalid JSON: {}", e)))?;
        ...
        if let Some(name) = obj.get("name").and_then(|v| v.as_str()) {
            if name.is_empty() || !name.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
                return Err(SkillError::InvalidInput(
                    "Skill name must be non-empty and contain only alphanumeric characters, hyphens, and underscores"
                        .to_string(),
                ));
            }
        }
        ...
        match skill_type {
            "code_inline" | "code_session" => {
                if !obj.contains_key("command") {
                    return Err(SkillError::InvalidInput(
                        "Code skills require 'command' field".to_string(),
                    ));
                }
            },
            "conversation" => {
                if !obj.contains_key("prompt_template") {
                    return Err(SkillError::InvalidInput(
                        "Conversation skills require 'prompt_template' field".to_string(),
                    ));
                }
            },
            "prompt_inline" => {
                if !obj.contains_key("prompt") {
                    return Err(SkillError::InvalidInput(
                        "Prompt inline skills require 'prompt' field".to_string(),
                    ));
                }
            },
            _ => {
                return Err(SkillError::InvalidInput(format!(
                    "Unknown skill type: {}. Valid types: code_inline, code_session, conversation, prompt_inline",
                    skill_type
                )));
            },
        }
    ...
    pub fn validate_parameters(
        params: &Value,
        param_defs: &[crate::cli::skills::types::Parameter],
    ) -> Result<(), SkillError> {
    ...
    fn validate_parameter_type(
        value: &Value,
        param_def: &crate::cli::skills::types::Parameter,
    ) -> Result<(), SkillError> {
                    ...
                    if let Some(allowed_values) = &param_def.values {
                        if !allowed_values.contains(&str_val.to_string()) {
                            return Err(SkillError::InvalidInput(format!(
                                ...
                                param_def.name, allowed_values
                            )));
                        }
                    }
                } else {
                    return Err(SkillError::InvalidInput(format!(
                        "Parameter '{}' must be a string for enum type",
                        param_def.name
                    )));
                }
            },
