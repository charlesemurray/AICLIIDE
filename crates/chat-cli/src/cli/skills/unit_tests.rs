#[cfg(test)]
mod tests {
    use crate::cli::skills::{Skill, SkillRegistry, SkillError, builtin::calculator::Calculator};
    use serde_json::json;
    ...
    async fn test_skill_registry() {
        ...
        let result = registry.execute_skill("calculator", json!({"a": 2, "b": 3, "op": "add"})).await.unwrap();
